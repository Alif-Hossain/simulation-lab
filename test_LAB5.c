...
int totalWaitingTime=0,totalIdleTime=0,wait_count=0;
struct simulation{
    int cust_no;
    int rd_ar;
    int intr_arr_time;
...
int main(){
   ...
   // FILE *out_file = fopen("test1.txt", "w"); // write only
    ...
    for(i=2;i<=n;i++){
        ...
        else if(s[i].rd_ar > 875 && s[i].rd_ar < 999) s[i].intr_arr_time=8;
    ...
    for(i=1;i<=n;i++){
        ...
        else if(s[i].rd_se > 95 && s[i].rd_se < 100) s[i].service_time=6;
   ...
   int total_spend_time = schedule_spend_time(s, n);
    ...
    printf("\n   ------------------------------------------------------------------------------------------------------------------");
    printf("\n  | Cust.| RD. for |Inter_arrival|Cu.Arrival| RD. for | Service | T.Service| T.Service| Waiting | Idle.T |T.Spend in|\n");
      printf("  |  (i) |Arrival.T| Time (IAT)  |Time(CAT) |Service.T| Time(ST)|Begins(SB)| End(SE)  | Time(WT)|  (IT)  |System(TS)|");
    printf("\n   ------------------------------------------------------------------------------------------------------------------\n");
    for(i=1; i<=n; i++){

        printf("     %d       %d         %d              %d          %d          %d       %d          %d            %d      %d       %d\n"
               ,s[i].cust_no,s[i].rd_ar,s[i].intr_arr_time,s[i].cu_arr_time,s[i].rd_se,s[i].service_time,s[i].service_begin,
    ...
    printf("\t\t\t\t\t\t\t    %d \t\t\t\t    %d\t   %d\t   %d",
           total_service_time,totalWaitingTime,totalIdleTime,total_spend_time);
    avg_waiting_time = (float)totalWaitingTime/(float)n;
    probability_wait = (float)wait_count/(float)n;
    probability_of_idle_server = (float)totalIdleTime/(float)s[n].time_ser_end;
    avg_service_time = (float)total_service_time/(float)n;
    avg_time_btn_arrival = (float)s[n].cu_arr_time/(float)(n-1);
    avg_waiting_time_those_who_wait = (float)totalWaitingTime/(float)wait_count;
...
void schedule_arrival_time (Simulation s[],int n){
    int i;
    for( i=2; i<=n; i++){
        s[i].cu_arr_time=s[i-1].cu_arr_time+s[i].intr_arr_time;
    }
}
void schedule_service_time (Simulation s[],int n){
    int i;
    for( i=2; i<=n; i++){
            if(s[i].cu_arr_time > s[i-1].time_ser_end){
                s[i].service_begin=s[i].cu_arr_time;
            }
            else{
...
int waiting_and_idle_time (Simulation s[],int n ){
    ...
    for( i=2; i<=n; i++){
            if(s[i].cu_arr_time > s[i-1].time_ser_end){
                s[i].idle_time=s[i].cu_arr_time-s[i-1].time_ser_end;
                totalIdleTime+=s[i].idle_time;
                s[i].waiting_time=0;
            }
            else{
                s[i].waiting_time=s[i-1].time_ser_end-s[i].cu_arr_time;
